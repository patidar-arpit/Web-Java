...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public Employee loadEmpDetailsById(Long empId) {
		...
		try {
			...
			//Hint for Hibernate to behave in an EAGER manner : access any non id getter
			 emp.getFirstName();//select  is fired giving hint to the HB that please fetch the data from db and intialize 
			// if we not write above line 106 so the proxy emp is un intialized and this methos return unintialized proxy
			 // and in tester if we access this proxy The LazyException comes
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
	...
	@Override
	public void assignAadharCardDetails(long id, String cardNo, String createdOn,String location) {
		
		Session session = getFactory().getCurrentSession();
		Transaction tx = session.beginTransaction();
		try {
			Employee e = session.get(Employee.class, id);
			AdhaarCard adhr = new AdhaarCard(cardNo,LocalDate.parse(createdOn),location);
			e.setCard(adhr);
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
	...
	@Override
	public void paymentDetail(long id, String type, String cvv, String expiryDate) {
	     
		Session session = getFactory().getCurrentSession();
		Transaction tx = session.beginTransaction();
		try {
		    Employee e = session.get(Employee.class, id);
			PaymentOption po = new PaymentOption(PaymentType.valueOf(type), cvv, LocalDate.parse(expiryDate));
			e.getPaymentOptions().add(po);
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
	...
	@Override
	public void assignSkillsOfEmployee(long id, List<String> skills) {
		Session session = getFactory().getCurrentSession();
		Transaction tx = session.beginTransaction();
		try {
		    Employee e = session.get(Employee.class, id);
